  ...
  setup () {
    ...
    const getCateList = async () => {
      ...
      if (res.meta.status !== 200) {
        return ElMessage.error('获取分类列表失败')
      }
    ...
    const getParamsData = async () => {
      ...
      if (res.meta.status !== 200) {
        return ElMessage.error('请求params失败')
      }
      ...
      if (addFormRef.value !== null) {
        addFormRef.value.validate(async (valid) => {
          ...
          if (res.meta.status !== 201) {
            return ElMessage.error('添加params失败')
          }
    ...
    const editor = async (info) => {
      ...
      if (res.meta.status !== 200) {
        return ElMessage.error('查询params失败')
      }
    ...
    const commitEdit = () => {
      if (editFormRef.value !== null) {
        editFormRef.value.validate(async (valid) => {
          ...
          if (res.meta.status !== 200) {
            return ElMessage.error('提交params失败')
          }
